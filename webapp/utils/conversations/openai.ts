...
const validateChaGPTConversations = (data: unknown): SafeParseReturnType<unknown, ChatGPTConversations> => ConversationsSchema.safeParse(data);
...
const importChatGPTConversation = (fromChatGPT: ChatGPTConversations): Conversation[] => {
    const conversations = fromChatGPT.map((conversation) => {
        ...
        let c = createdAt;
        let u = updatedAt;
        const messages = Object.values(conversation.mapping).map((mapping) => {
