...
export const hasNewMessages = (messagesA: Message[] | undefined, messagesB: Message[] | undefined) => {
  if (messagesA && messagesA.length === messagesB?.length) {
    return messagesA.every((message, index) => {
      const messageB = messagesB[index];
      return (message.updatedAt < messageB.updatedAt && (message.id === messageB.id && deepEqual(message.author, messageB.author)
